...
const MemoExample = () => {
  return (
    <>
      ...
      <CollapseWrapper>
        <BlockQuoteWrapper>
          <p>
            ...
            <br />
            Если ваш компонент всегда рендерит одно и то же при неменяющихся пропсах, вы можете
            обернуть его в вызов React.memo для повышения производительности в некоторых случаях,
            мемоизируя тем самым результат. Это значит, что React будет использовать результат
            последнего рендера, избегая повторного рендеринга.
            <br />
            Говоря просто, компонент высшего порядка — это функция, которая принимает компонент и
      ...
      <CardWrapper>
        <i className="bi bi-lightbulb"></i> Если функциональный компонент обёрнут в{' '}
        <code>React.memo</code> и использует <code>useState, useReducer или useContext</code>, он
        будет повторно рендериться при изменении состояния или контекста.
      </CardWrapper>
      <CardWrapper>
      ...
      <CardWrapper>
        ...
        <CardWrapper>
          <i className="bi bi-lightbulb"></i> Если функция для сверки не задана, то выполняется
          поверхностная сверка по{' '}
          <Link
            to={{
              pathname:
